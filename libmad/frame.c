...
emscripten_log(1,"mad_header_decode bitrate:%d samplerate:%d channels:%d %d",header->bitrate,header->samplerate,header->mode,N);
   //verify there is enough data left in buffer to decode this frame 
  if (N + MAD_BUFFER_GUARD > end - stream->this_frame) {
    //emscripten_log(1,"mad_header_decode %d %d %d",stream->this_frame,end,N);
...
emscripten_log(0,"success:%d",result);
  return 0;

 fail:
 emscripten_log(0,"fail:%d",stream->error);
  stream->anc_bitlen = 0;
  return -1;
}
