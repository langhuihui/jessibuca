...
uint16_t drm_ps_data(drm_ps_info *ps, bitfile *ld)
{
    ...
    ps->bs_enable_sa = faad_get1bit(ld DEBUGVAR(1,0,""));
    ps->bs_enable_pan = faad_get1bit(ld DEBUGVAR(1,0,""));
...
static void drm_ps_sa_element(drm_ps_info *ps, bitfile *ld)
{
    ...
    ps->bs_sa_dt_flag = faad_get1bit(ld DEBUGVAR(1,0,""));
    if (ps->bs_sa_dt_flag)
    {
        huff = t_huffman_sa;
    } else {
...
static void drm_ps_pan_element(drm_ps_info *ps, bitfile *ld)
{
    ...
    ps->bs_pan_dt_flag = faad_get1bit(ld DEBUGVAR(1,0,""));
...
static int8_t huff_dec(bitfile *ld, drm_ps_huff_tab huff)
{
    ...
    while (index >= 0)
    {
        bit = (uint8_t)faad_get1bit(ld DEBUGVAR(1,0,""));
